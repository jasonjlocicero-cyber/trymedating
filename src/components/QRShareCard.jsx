...
export default function QRShareCard({
  ...
  const copyLink = async () => {
    try {
      ...
      alert(inviteUrl);// src/components/QRShareCard.jsx
...
export default function QRShareCard({
  ...
  useEffect(() => {
    ...
    async function makeQR() {
      ...
      if (!inviteUrl) return;
      try {
        const QR = await import("qrcode"); // dynamic so app still builds if dep missing
        const url = await QR.toDataURL(inviteUrl, {
          width: size,
          margin: 1,
          color: { dark, light },
