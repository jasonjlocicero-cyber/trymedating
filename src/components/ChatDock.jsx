...
export default function ChatDock({ me, partnerId, partnerName = '', onClose, onUnreadChange = () => {} }) {
  ...
  // signed URL cache: message.id -> signed URL
  const [signedUrlByMsgId, setSignedUrlByMsgId] = useState({})
  ...
  useEffect(() => {
    ...
    for (const m of messages) {
      if ((m.kind === 'image' || m.kind === 'file') && m.media_path && !signedUrlByMsgId[m.id]) {
        need.push({ id: m.id, path: m.media_path })
      }
    }
