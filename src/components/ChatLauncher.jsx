...
/**
 * Error boundary so if ChatDock throws (like "panel is not defined"),
 * you get a friendly box instead of a full blank page.
 */
class DockErrorBoundary extends React.Component {
  ...
  componentDidCatch(err, info) {
    console.error('[ChatDock crashed]', err, info);
  }
  render() {
    if (this.state.err) {
      return (
        ...
        >
          <div style={{ fontWeight: 900, marginBottom: 6 }}>Chat error</div>
          <div className="muted" style={{ marginBottom: 10, lineHeight: 1.4 }}>
            The chat view crashed with:
            <div style={{ marginTop: 6, fontFamily: 'monospace', fontSize: 12, color: '#b91c1c' }}>
          ...
          <div className="muted" style={{ marginTop: 10, fontSize: 12 }}>
            Open DevTools → Console to see the exact file/line causing it.
          </div>
        </div>
      );
    }
...
export default function ChatLauncher({ onUnreadChange = () => {} }) {
  ...
  async function computeUnread(userId) {
    ...
    // NOTE: when head:true, Supabase returns count in "count", not in data.length
  ...
  return (
    <>
      ...
      {open && canChat && (
        <DockErrorBoundary
          onReset={() => {
            // go back to the recent list instead of blank-screening
            setPartnerId(null);
            setPartnerName('');
          }}
          onClose={() => {
        ...
        >
          <ChatDock
            partnerId={partnerId}
            // partnerName isn't required by ChatDock, but we keep it in state for future header use
